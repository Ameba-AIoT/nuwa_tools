...
class PREPEND_TOOL:
    ...
    def prepend_header(self):
        ...
        HEADER_FINAL=''
        if IMAGE_FILENAME_NEW == "ram_1.bin" or IMAGE_FILENAME_NEW == "xip_boot.bin":
...
if sys.argv[1] == "keypair":
    if len(sys.argv) < 4:
        print_help()
        sys.exit()
    sboot = secure_boot(len(sys.argv), sys.argv)
...
elif sys.argv[1] == "rsip":
    if len(sys.argv) < 7:
        print_help()
        sys.exit()
    rsip = RSIP(len(sys.argv), sys.argv)
    rsip.RSIP_ImageEncrypt()
elif sys.argv[1] == "rdp":
    if len(sys.argv) < 6:
        print_help()
        sys.exit()
    rdp = RDP(len(sys.argv), sys.argv)
    rdp.rdp_encrypt()
elif sys.argv[1] == "huk":
    sboot = secure_boot(len(sys.argv), sys.argv)
